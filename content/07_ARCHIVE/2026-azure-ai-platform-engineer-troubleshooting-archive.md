...
PRD `apprdaifabric`에서 F32 상태와 Rolling 평균 `[22.81% × 5]`를 기준으로 Scale Down 조건이 충족되어 F16으로 변경되는 실제 실행 로그를 확인했습니다. 적용 후 약 일주일 동안 Schedule CronJob과 Dynamic CronJob의 충돌 없이 동작하는지 모니터링했습니다.
...
### Symptom
`apprdaiacrgenai.azurecr.io/apne2-devops-demo-aibe-ok-release:253961` Image Pull에서 `ImagePullBackOff / NotFound`가 발생했습니다.
