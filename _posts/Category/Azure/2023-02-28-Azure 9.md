...
# 2. 클러스터 설치
## [1]. 운영 체제를 구성합니다.
1. Pacemaker는 경우에 따라 허용된 수를 모두 사용할 수 있는 많은 프로세스를 만듭니다.    
이 경우 클러스터 노드 간의 하트비트가 실패하고 리소스의 장애 조치(failover)가 발생할 수 있습니다.    
다음 매개 변수를 설정하여 허용되는 최대 프로세스 수를 늘리는 것이 좋습니다.
```
...
## [2]. 고가용성 클러스터에 대해 cloud-netconfig-azure를 구성합니다.
* 클라우드 네트워크 플러그 인이 가상 IP 주소를 제거하는 것을 방지하려면 (Pacemaker가 할당을 제어해야 함)   
...
2 | NO2 SERVER keygen 생성
3 | NO2 > NO1 SERVER의 **authorized_keys**에 복사
4 | NO1 > NO2 SERVER의 **authorized_keys**에 복사
...
# copy the public key 확인
sudo cat /root/.ssh/id_rsa.pub
```

## [3-2]. [AP-NO2, DP-NO2] NO2 SERVER의 ssh-keygen 생성 후 authorized keys에 NO1 SERVER의 keygen 붙여넣기
```
# keygen 생성
sudo ssh-keygen
...
## [3-3]. [AP-NO1, DP-NO1] NO2 SERVER의 ssh-keygen을 복사해서 NO1 SERVER의 authorized keys에 붙여넣기
```
...
## [1]. [AP-NO1, DP-NO1] 펜싱에 Azure 펜스 에이전트를 사용하는 경우 다음 명령을 실행합니다. 
...
## [2]. 펜스 에이전트에 관리 ID 를 사용하는 경우 다음 명령을 실행합니다.
```
