...
export default function ProjectModalList({projects}:{projects:ProjectItem[]}){
 ...
 return <div className={`projectOps ${active?"isReading":"isBoard"}`}>
  ...
  {active?<article className="projectOpsReader">
   ...
   {filtered.length>0&&<div className="projectOpsPagination"><span>SHOWING {(safePage-1)*pageSize+1}–{Math.min(safePage*pageSize,filtered.length)} OF {filtered.length} RECORDS</span><div><button type="button" disabled={safePage===1} onClick={()=>setPage(p=>Math.max(1,p-1))}>‹</button>{Array.from({length:pageCount},(_,i)=>i+1).map(n=><button type="button" key={n} className={n===safePage?"active":""} onClick={()=>setPage(n)}>{n}</button>)}<button type="button" disabled={safePage===pageCount} onClick={()=>setPage(p=>Math.min(pageCount,p+1))}>›</button></div></div>}
  </>}
 </div>;
}
