 ...
 return <div className={`projectOps ${active?"isReading":"isBoard"}`}>
  ...
  {active?<article className="projectOpsReader">
   ...
   {filtered.length?<div className="projectOpsBoard">
    ...
    {visible.map((project,index)=><button type="button" className="projectOpsRow" key={project.slug} onClick={()=>open(project)}>
   ...
   {filtered.length>0&&<div className="projectOpsPagination"><div className="projectOpsPageMeta"><span>SHOWING <b>{String((safePage-1)*pageSize+1).padStart(2,"0")}–{String(Math.min(safePage*pageSize,filtered.length)).padStart(2,"0")}</b> OF <b>{String(filtered.length).padStart(2,"0")}</b> RECORDS</span><span>PAGE <b>{String(safePage).padStart(2,"0")}</b> / {String(pageCount).padStart(2,"0")}</span></div><div className="projectOpsPageControls"><button type="button" className="pageNav" disabled={safePage===1} onClick={()=>setPage(p=>Math.max(1,p-1))}>← <span>PREV</span></button><div className="pageNumbers">{Array.from({length:pageCount},(_,i)=>i+1).map(n=><button type="button" key={n} aria-label={`Go to page ${n}`} aria-current={n===safePage?"page":undefined} className={n===safePage?"active":""} onClick={()=>setPage(n)}>{String(n).padStart(2,"0")}</button>)}</div><button type="button" className="pageNav" disabled={safePage===pageCount} onClick={()=>setPage(p=>Math.min(pageCount,p+1))}><span>NEXT</span> →</button></div></div>}
  </>}
 </div>;
}
