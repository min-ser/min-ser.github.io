...
export type TimelineCareer={kind?:"career"|"training";id:string;slug:string;company:string;position:string;startDate:string;endDate?:string|null;status?:string;roles:string[];skills:string[];markdown:string;projects:TimelineProject[];};
...
export default function CareerTimeline({careers,config}:{careers:TimelineCareer[];config:TimelineConfig}){
 ...
 return <div className="careerIntelligence">
  <div className="careerIntelligenceTelemetry" aria-label="Career intelligence summary">
   <div><span>CAREER RECORDS</span><strong>{String(intelligence.careers).padStart(2,"0")}</strong></div>
   <div><span>TRAINING</span><strong>{String(intelligence.trainings).padStart(2,"0")}</strong></div>
   <div><span>LINKED PROJECTS</span><strong>{String(intelligence.projects).padStart(2,"0")}</strong></div>
   <div><span>TECHNOLOGIES</span><strong>{String(intelligence.technologies).padStart(2,"0")}</strong></div>
  </div>
